...
my $finder = sub {
  ...
  if(defined $z && $modality ne "RTDOSE"){
    if(defined $ImagesByZ{$z}){
      print "Error: two Images at $z: $ImagesByZ{$z} and $file_name\n";
    }
    $ImagesByZ{$z} = $file_name;
    $ImagesByFile{$file_name} = $UID;
    if(defined $ImagesByUid{$UID}){
      print "Error: Two Images with same UID ($UID):\n" .
  ...
  # is it an RT Struct?
    ...
    if(exists $SS{$UID}){
      print "Error: Two SS's with same UID ($UID):\n" .
        "\t$file_name\n\t$SS{$UID}->{file}\n";
      return;
    }
    $SS{$UID} = {
    ...
    for my $i (@{$ct_vol_ref->{list}}){
      ...
      if(exists($SS{$UID}->{Volume}->{$ref_sop_inst})){
        print "Error: $UID is referenced more than once in " .
    ...
    for my $i (@{$ct_cont_ref->{list}}){
      ...
      unless($geo_type eq "CLOSED_PLANAR"){
        print "Ignoring roi $roi_i, contour $cont_i, " .
      ...
      if(exists($SS{$ss}->{Volume}->{$ref_sop_inst})){
        ...
        if(exists($ImagesByUid{$ref_sop_inst})){
          ...
          if($diff <= 0.1){
            ...
            print "Error:  Difference between z in contour and " .
...
for my $uid (keys %Plan){
  my $plan = $Plan{$uid};
  if($plan->{ref_ss_class} eq "1.2.840.10008.5.1.4.1.1.481.3"){
