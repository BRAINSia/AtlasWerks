...

{
  ...
  sub new{
    ...
    if(exists $this{base_uid}){
      ...
      $this{base_uid} = Posda::UID::GetPosdaRoot($arg);
      $this{posda_args} = $arg;
    }
    my $for_i = 0;
  ...
  my $GenerateSeriesOfImages = sub{
    ...
    for my $slice_no (1 .. $num_slices){
      ...
      for my $i (keys %$config){
        $ds->InsertElementBySig($i, $config->{$i});
      }
      $ds->InsertElementBySig("(0008,1030)", $study_description);
      $ds->InsertElementBySig("(0020,0052)", $for_uid);
      $ds->InsertElementBySig("(0020,000d)", $study_uid);
      $ds->InsertElementBySig("(0020,000e)", $series_uid);
      $ds->InsertElementBySig("(0020,0011)", $series_id);
      $ds->InsertElementBySig("(0020,0010)", $study_id);
      $ds->InsertElementBySig("(0008,0018)", $sop_instance);
      $ds->InsertElementBySig("(0020,0013)", $current_instance_no);
      $ds->InsertElementBySig("(0020,0032)", $current_ipp);
...
#          $this, $rows, $cols, $iop, $current_ipp, $pix_sp, $for_id, $obj_list);
      $ds->InsertElementBySig("(7fe0,0010)", $pixel_data);
      $ds->InsertElementBySig("(0028,0103)", $pixel_representation);
      if(defined $slope){
        $ds->InsertElementBySig("(0028,1053)", $slope);
        $ds->InsertElementBySig("(0028,1052)", $intercept);
      }
...
print "File_name: $file_name\n";
  ...
  my $GenerateRegSeries = sub{
    ...
    for my $arg (@$args){
      ...
      $ds->InsertElementBySig("(0008,1030)", $study_description);
      $ds->InsertElementBySig("(0020,0052)", $to_uid);
      $ds->InsertElementBySig("(0020,000d)", $study_uid);
      $ds->InsertElementBySig("(0020,000e)", $series_uid);
      $ds->InsertElementBySig("(0020,0011)", $series_id);
      $ds->InsertElementBySig("(0020,0010)", $study_id);
      $ds->InsertElementBySig("(0008,0018)", $sop_instance);
      $ds->InsertElementBySig("(0020,0013)", 1);

      $ds->InsertElementBySig("(0070,0308)[0](0020,0052)", $to_uid);
      $ds->InsertElementBySig("(0070,0308)[1](0020,0052)", $from_uid);
...
print "File_name: $file_name\n";
  ...
  my $GenerateStructSeries = sub{
    ...
    $ds->InsertElementBySig("(3006,0010)[0](0020,0052)", $ref_for);
    $ds->InsertElementBySig("(3006,0010)[0](3006,0012)[0](0008,1150)",
       "1.2.840.10008.3.1.2.3.1");
    $ds->InsertElementBySig("(3006,0010)[0](3006,0012)[0](0008,1155)",
       $ref_study_id);
    ...
    for my $roi_num (sort {$a <=> $b} keys %$roi_hash){
      $ds->InsertElementBySig("(3006,0020)[$i](3006,0022)", $roi_num);
      $ds->InsertElementBySig("(3006,0020)[$i](3006,0026)",
         $roi_hash->{$roi_num}->{name});
      $ds->InsertElementBySig("(3006,0020)[$i](3006,0036)",
         $roi_hash->{$roi_num}->{gen_alg});
      $ds->InsertElementBySig("(3006,0020)[$i](3006,0024)",
         $ref_for);
  ...
  my $GeneratePlanSeries = sub {
    ...
    $ds->InsertElementBySig("(300c,0060)[0]>(0008,1150)", $TypeToSop->{RTS});
    $ds->InsertElementBySig("(300c,0060)[0]>(0008,1155)", $ss_uid);
    ##
    #  Create Setup Info
    ##
    $ds->InsertElementBySig("(300a,0180)[0](0018,5100)", 'HFS');
    $ds->InsertElementBySig("(300a,0180)[0](300a,0182)", 1);
    $ds->InsertElementBySig("(300a,0180)[0](300a,01b0)", "ISOCENTRIC");
    ##
    #  Create Fraction Group Info
    ##
    $ds->InsertElementBySig("(300a,0070)[0](300a,0071)", 1);
    $ds->InsertElementBySig("(300a,0070)[0](300a,0078)", 1);
    $ds->InsertElementBySig("(300a,0070)[0](300a,0080)", $num_beams);
    $ds->InsertElementBySig("(300a,0070)[0](300a,00a0)", 0);
    my @beams = sort {
    ...
    for my $beam_i (0 .. $#beams){
      ...
      $ds->InsertElementBySig("(300a,00b0)[$beam_i](300a,00b2)", "Bogus");
      $ds->InsertElementBySig(
        "(300a,00b0)[$beam_i](300a,00b3)", 'MU');
      $ds->InsertElementBySig(
        "(300a,00b0)[$beam_i](300a,00b4)", 1000.0);
      $ds->InsertElementBySig(
        "(300a,00b0)[$beam_i](300a,00b6)[0](300a,00b8)", 'X');
      $ds->InsertElementBySig(
        "(300a,00b0)[$beam_i](300a,00b6)[0]>(300a,00bc)", 1);
      $ds->InsertElementBySig(
        "(300a,00b0)[$beam_i](300a,00c0)", 1);
      $ds->InsertElementBySig(
        "(300a,00b0)[$beam_i](300a,00c2)", "Bogus");
      $ds->InsertElementBySig(
        "(300a,00b0)[$beam_i](300a,00c3)", "Bogus beam");
      $ds->InsertElementBySig(
      ...
      $ds->InsertElementBySig(
        "(300a,00b0)[$beam_i](300a,0111)[0](300a,011a)[0](300a,00ba)", 'X');
      $ds->InsertElementBySig(
        "(300a,00b0)[$beam_i](300a,0111)[0](300a,011a)[0](300a,011c)", 
        [5.0, 5.0]);
      $ds->InsertElementBySig(
        "(300a,00b0)[$beam_i](300a,0111)[0](300a,011e)", 0.0);
      $ds->InsertElementBySig(
        "(300a,00b0)[$beam_i](300a,0111)[0](300a,011f)", "CW");
      ##### IsoCenter
      $ds->InsertElementBySig(
        "(300a,00b0)[$beam_i](300a,0111)[0](300a,012c)",
        $this->{beam_doses}->{$beam_name}->{target}->{center}
      );
      $ds->InsertElementBySig(
  ...
  my $GenerateDoseSeries = sub {
    ...
    for my $i (@{$args->{doses}}){
      ...
      my $ds = Posda::Dataset->new_blank();
      $ds->InsertElementBySig("(300c,0002)[0]>(0008,1150)", $TypeToSop->{RTP});
      $ds->InsertElementBySig("(300c,0002)[0]>(0008,1155)", $ref_plan_uid);
      if(defined $ref_beam_no){
        $ds->InsertElementBySig("(300c,0006)", $ref_beam_no);
      }
      for my $i (keys %$config){
        $ds->InsertElementBySig($i, $config->{$i});
      }
      ...
      $ds->InsertElementBySig("(0020,0010)", $study_id);
      $ds->InsertElementBySig("(0008,0018)", $sop_instance);
      $ds->InsertElementBySig("(0028,0008)", $num_slices);
      $ds->InsertElementBySig("(0028,0009)", [0x3004,0x000c]);
      my @gfov;
      my $offset = 0;
      for my $i (1 .. $num_slices){
         push(@gfov, $offset);
      ...
      $ds->InsertElementBySig("(3004,000c)", \@gfov);
      ...
      $ds->InsertElementBySig("(3004,000a)", $dose_summation_type);
      my $pixels = "";
      ...
      $ds->InsertElementBySig("(0020,0032)", $init_ipp);
      $ds->InsertElementBySig("(7fe0,0010)", $pixels);
      my $modality = $config->{"(0008,0060)"};
      my $file_name = "$dest_dir/${modality}_$sop_instance.dcm";
      $ds->WritePart10(
        $file_name, '1.2.840.10008.1.2.1', 'POSDA_PHNTM', undef, undef
