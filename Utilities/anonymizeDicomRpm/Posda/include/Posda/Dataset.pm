...
sub RenderText{
  ...
  if(length($value) > $max_len){
    if($numeric){
      ...
      $value =~ /^(.{$max_len})/;
      my $new_value = $1;
      print STDERR "truncating text $value to $new_value\n";
      return $new_value;
    }
  } else {
    return $value;
...
sub EncodeTextEle{
  my($value, $pad, $max_len, $numeric) = @_;
  ...
  if(ref($value) eq "ARRAY"){
    ...
    for my $i (0 .. $#{$value}){
      if(defined($value->[$i])){
        $all .= RenderText($value->[$i], $max_len, $numeric);
        #$all .= $value->[$i];
      }
      unless($i == $#{$value}){
        $all .= "\\";
    ...
    $all = RenderText($value, $max_len, $numeric);
  }
  if(length($all) % 2){
    $all .= $pad;
  }
  ...
  for my $grp (sort { $a <=> $b } keys %$ds ){
    if($grp & 1){
      unless(exists $ds->{$grp}->{private}){
        ($ds->{$grp}->{private}, $ds->{$grp}->{private_map}) = 
          $ds->OldConvertToPrivate($grp);
      }
...
sub DumpAT{
  ...
  for my $i (0 .. $#{$in_value}){
    my $grp = ($in_value->[$i] & 0xffff0000) >> 16;
    my $ele = ($in_value->[$i] & 0xffff);
    printf("(%04x,%04x)", $grp, $ele);
    unless($i == $#{$in_value}){
      print "\\";
    }
  }
...
sub DumpEleLen{
  ...
  if(defined($ele->{type}) &&  $ele->{type} eq "text") {
    ...
    print "can't figure length of element";
  }
}
  ...
  if(defined $before->{VR} && $before->{VR} ne "UN"){
    if($ele_info->{VR} ne "UN" && $ele_info->{VR} ne $before->{VR}){
      for my $key (keys %$ele_info){
        print "ele_info->{$key} = $ele_info->{$key}\n";
      }
      print "Explicit VR ($before->{VR}) doesn't match defined DD VR" .
      " ($ele_info->{VR})\n";
    ...
    if($ele < 256){
      ...
      unless(defined $owners{$block}){
        if(exists $DD->{ReverseDefaultPrivateGroupTags}->{$block}){
          ...
          $owners{$block} = "Yikes:$block";
          $r_owners{"Yikes:$block"} = $block;
        }
      }
      my $sig = sprintf("(%04x,\"%s\",%02x)", $grp, $owners{$block}, $pvt_ele);
      my $ele_info = $DD->get_ele_by_sig($sig);
      unless(defined $ele_info){
        $ele_info = {
          VR => 'UN',
...
sub ConvertToPrivate{
  ...
  for my $ele (sort { $a <=> $b} keys %{$ds->{$grp}}){
    ...
    if($ele < 256){
      ...
      my $block = int($ele/256);
      my $pvt_ele = $ele % 256;
      unless(defined $r_owners{$block}){
        if(exists $DD->{ReverseDefaultPrivateGroupTags}->{$block}){
          my $owner = $DD->{ReverseDefaultPrivateGroupTags}->{$block};
          $r_owners{$owner} = $block;
          $owners{$block} = $owner;
        } else {
          my $owner = "Yikes:$block";
          $r_owners{$owner} = $block;
          $owners{$block} = $owner;
        }
      }
  ...
  $ds->{$grp}->{r_private_map} = { reverse %owners };
}
sub ConvertFromPrivate{
  my($ds, $grp) = @_;
  my $private_map = $ds->{$grp}->{private_map};
  ...
  for my $i (sort { $a cmp $b } keys %$private){
    my $c_ele;
    if(exists $r_private_map{$i}){
      $c_ele = $r_private_map{$i};
    } else {
      my $t_ele = 0x10;
      ele:
      while(
  ...
  if($remain eq ""){
    ...
    if(defined $owner){
      unless(exists $this->{$grp}->{private_map}->{$owner}){
        ...
        if(exists $DD->{DefaultPrivateGroupTags}->{$owner}){
          $block = $DD->{DefaultPrivateGroupTags}->{$owner};
          if(exists $this->{$grp}->{r_private_map}->{$block}){
            die "Default Block $block allocated for " .
...
sub Try{
  ...
  if($@){
    ...
    unless($xfr_stx eq "1.2.840.10008.1.2"){
      push(@{$parser->{errors}}, "No metaheader with xfr_stx: $xfr_stx");
    }
  } else {
    $size = $parser->{file_length};
