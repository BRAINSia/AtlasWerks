...
sub RtPatientSetup{
  ...
  for my $m (@{$s->{list}}){
    ...
    my $m1 = $ds->Substitutions("(300a,0180)[$i](300a,0401)[<0>](0008,1155)");
    for my $s1 (@{$m1->{list}}){
      ...
      #    rt_plan_setup_device
      my $s4 = $ds->Substitutions("(300a,0180)[$i](300a,1b4)[<0>](300a,01b6)");
      for my $m4 (@{$s4->{list}}){
        my $j = $m4->[0];
        ...
        my $psd_parms = {
          setup_device_type => "(300a,0180)[$i](300a,1b4)[$j](300a,01b6)",
          setup_device_label => "(300a,0180)[$i](300a,1b4)[$j](300a,01b8)",
          setup_device_description =>
            "(300a,0180)[$i](300a,1b4)[$j](300a,01ba)",
          setup_device_parameter => "(300a,0180)[$i](300a,1b4)[$j](300a,01bc)",
          setup_reference_description =>
            "(300a,0180)[$i](300a,1b4)[$j](300a,01d0)"
        };
...
sub RtFractionScheme{
  ...
  print "RtFractionScheme Module not yet implemented\n";
}
sub RtBeams{
  my($db, $ds, $id, $hist, $errors) = @_;
  #  Set up following tables:
...
sub RtBrachy{
  my($db, $ds, $id, $hist, $errors) = @_;
  print "RtBrachy Module not yet implemented\n";
}
sub CRImage{
  my($db, $ds, $id, $hist, $errors) = @_;
...
sub ContrastBolus{
  my($db, $ds, $id, $hist, $errors) = @_;
  print "ContrastBolus Module not yet implemented\n";
}
