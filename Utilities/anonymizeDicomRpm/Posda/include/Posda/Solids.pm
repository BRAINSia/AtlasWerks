...
}
{
  ...
  @ISA = ("Posda::Solids");
  sub new{
    my($class, $center, $height, $width, $length) = @_;
  ...
  sub In {
    my($this, $point) = @_;
    if(
      $point->[0] < $this->{tfrhc}->[0] &&
      $point->[0] > $this->{bblhc}->[0] &&
      $point->[1] < $this->{tfrhc}->[1] &&
      $point->[1] > $this->{bblhc}->[1] &&
      $point->[2] < $this->{tfrhc}->[2] &&
      $point->[2] > $this->{bblhc}->[2]
    ){
      return 1;
    }
    return 0;
  ...
  sub In_CCode{
    ...
    $ccg->add_body("(\n");
    $ccg->add_indent("  ");
    $ccg->add_body("(x < ($this->{tfrhc}->[0]))\n");
    $ccg->sub_indent("  ");
    $ccg->add_body("&&\n");
    $ccg->add_indent("  ");
    $ccg->add_body("(x > ($this->{bblhc}->[0]))\n");
    $ccg->sub_indent("  ");
    $ccg->add_body("&&\n");
    $ccg->add_indent("  ");
    $ccg->add_body("(y < ($this->{tfrhc}->[1]))\n");
    $ccg->sub_indent("  ");
    $ccg->add_body("&&\n");
    $ccg->add_indent("  ");
    $ccg->add_body("(y > ($this->{bblhc}->[1]))\n");
    $ccg->sub_indent("  ");
    $ccg->add_body("&&\n");
    $ccg->add_indent("  ");
    $ccg->add_body("(z < ($this->{tfrhc}->[2]))\n");
    $ccg->sub_indent("  ");
    $ccg->add_body("&&\n");
    $ccg->add_indent("  ");
    $ccg->add_body("(z > ($this->{bblhc}->[2]))\n");
    $ccg->sub_indent("  ");
    $ccg->add_body(")\n");
  }
  sub Dist{
...
}
{
  ...
  sub In_CCode{
    ...
    $ccg->add_body("$pz < ($this->{max_x}) &&\n");
    $ccg->add_body("$pz > ($this->{min_x})) ? 1 : 0");
    $ccg->add_body("))\n");
  }
  sub Dist{
    my($this, $point) = @_;
