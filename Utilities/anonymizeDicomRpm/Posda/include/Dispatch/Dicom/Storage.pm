...
sub finalize_command{
  ...
  if ($this->{finalized_command}->{"(0000,0100)"} == 0x01){
    ...
    my $AETitle = $dcm_conn->{descrip}->{ae_title};
    my $ImpClassUID = $dcm_conn->{descrip}->{imp_class_uid};
    my $ImpVerName = $dcm_conn->{descrip}->{imp_ver_name};
    my $file_name = "$SOPInstance.dcm";
    if($file_name eq ".dcm"){
...
sub finalize_ds{
  ...
  if($this->{finalized_command}->{"(0000,1000)"} eq ""){
    ...
    if($uid ne ""){
      ...
      ##  uncomment following line to descend into madness:
