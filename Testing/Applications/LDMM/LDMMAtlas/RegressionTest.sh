...
TEST_DIR="$TESTING_DIR/Applications/LDMM/LDMMAtlas"
...
if [ $RUN_ATLAS = "True" ]; then
    ...
    echo "doing warp..."
    _execute_save_output "$BIN_DIR/LDMMAtlas -f $PARAMFILE $*" LDMMAtlas.out
    echo "done."
    
    cat LDMMAtlas.out | sed -n 's/.*Scale \(.\)\/. Iter \(.*\) energy: Energy = \(.*\) (total) = \(.*\) (image) + \(.*\) (vec.*/\1 \2 \3 \4 \5/p' > LDMMAtlas.dat
...
_gen_plot "$TMP_DIR/LDMMAtlas.dat" "AtlasEnergy.png" "LDMM Atlas Energy"
