...
TEST_DIR="$TESTING_DIR/Applications/LDMM/LDMMWarp"
...
if [ $RUN_WARP = "True" ]; then
    ...
    # Run warp
    #
    PARAMFILE="$TEST_DIR/LDMMWarpParams.xml"
    echo "doing warp..."
    _execute_save_output "$BIN_DIR/LDMMWarp -f $PARAMFILE $*" LDMMWarp.out
    echo "done."
    
    #
    # Extract the energy from the output
    #
    cat LDMMWarp.out | sed -n 's/.*Scale \(.\), Iter \(.*\) Energy = \(.*\) (total) = \(.*\) (image) + \(.*\) (vec.*/\1 \2 \3 \4 \5/p' > LDMMWarp.dat
...
#
_gen_plot "$TMP_DIR/LDMMWarp.dat" "WarpEnergy.png" "LDMM Warp Energy"
