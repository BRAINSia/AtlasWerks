...
TEST_DIR="$TESTING_DIR/Applications/LDMM/LDMMAdShootingWarp"
...
if [ $RUN_WARP = "True" ]; then
    ...
    # Run warp
    #
    PARAMFILE="$TEST_DIR/LDMMAdShootingWarpParams.xml"
    echo "doing warp..."
    _execute_save_output "$BIN_DIR/LDMMWarp -f $PARAMFILE --ShootingOptimization true --UseAdjointShootingOptimization true $*" LDMMAdShootingWarp.out
    echo "done."
    
    #
    # Extract the energy from the output
    #
    cat LDMMAdShootingWarp.out | sed -n 's/.*Scale \(.\), Iter \(.*\), Energy = \(.*\) (total) = \(.*\) (image) + \(.*\) (vec.*/\1 \2 \3 \4 \5/p' > LDMMAdShootingWarp.dat
...
#
_gen_plot "$TMP_DIR/LDMMAdShootingWarp.dat" "WarpEnergy.png" "LDMM Ad Shooting Warp Energy"
