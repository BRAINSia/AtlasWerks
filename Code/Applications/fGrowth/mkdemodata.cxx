...
{
  std::string outputdir("/home/sci/jacob/tmp/spacetimedemo/forRecon-transPhantTrace");
  std::string imagePrefix("/home/sci/jacob/tmp/spacetimedemo/trueimgs/trueimg");
  std::string indexfilein("/home/sci/jacob/Data/SpaceTime/PhantomISonly/forRecon/index.csv");
  std::string indexfileout(outputdir + "/index.csv");

  // Set limits to get only certain slices (inclusive)
  //float zmin = -53.5; // for 1125 (gives 32 slices)
  //float zmax = -14.75;
  //float deltaz = 1.25;
  
  float zmin = -48.75; // for PhantomISonly (this is all of them)
  float zmax = 50.00;
  float deltaz = 1.25;
  

  unsigned int N = 32; // Size of slices acquired
  unsigned int Nz = 80; // number of z positions (for PhantomISonly)
//  unsigned int numslices = 2000;
  ...
  // Write base image for amplitude 0.5
        ...
        for (int k = 0; k < N*N; ++k)
          {
            ...
            short value = defImg(x,y,zf,amp,N,Nz);
